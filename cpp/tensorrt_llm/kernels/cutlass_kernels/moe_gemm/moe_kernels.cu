...
// ============================== NEW: Mixed-Precision Sort ==============================
// Assigns precision (bf16 vs fp4) to each expert based on token load.
// Single-thread kernel — E is small (typically 8-256), runs in microseconds.
__global__ void sortExpertsByTokenCountKernel(
    int64_t const* __restrict__ expert_first_token_offset,
    int const num_experts_per_node,
    int const num_high_precision_experts,
    int* __restrict__ expert_precision_assignment,
    int* __restrict__ bf16_expert_indices,
    int* __restrict__ fp4_expert_indices,
    int64_t* __restrict__ bf16_expert_first_token_offset,
    int64_t* __restrict__ fp4_expert_first_token_offset)
{
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
    cudaGridDependencySynchronize();
#endif
    ...
    int const E = num_experts_per_node;
    int const K = min(num_high_precision_experts, E);

    // Use dynamic shared memory for tokens_per_expert and selection flag
    extern __shared__ char smem[];
    int64_t* tokens_per_expert = reinterpret_cast<int64_t*>(smem);
    bool* selected = reinterpret_cast<bool*>(tokens_per_expert + E);

    // Step 1: Compute tokens per expert
    for (int e = 0; e < E; e++)
    {
        tokens_per_expert[e] = expert_first_token_offset[e + 1] - expert_first_token_offset[e];
        selected[e] = false;
    }

    // Step 2: Top-K selection — find K experts with highest token count → bf16
    int bf16_count = 0;
    for (int k = 0; k < K; k++)
    {
        int best = -1;
        ...
        for (int e = 0; e < E; e++)
        {
            if (!selected[e] && tokens_per_expert[e] > best_count)
            {
                best_count = tokens_per_expert[e];
                best = e;
            }
        }
        if (best >= 0)
        {
            selected[best] = true;
            expert_precision_assignment[best] = 1; // bf16
            bf16_expert_indices[bf16_count] = best;
            bf16_count++;
        }
    }

    // Step 3: Remaining experts → fp4
    int fp4_count = 0;
    for (int e = 0; e < E; e++)
    {
        if (!selected[e])
        {
            expert_precision_assignment[e] = 0; // fp4
            fp4_expert_indices[fp4_count] = e;
            fp4_count++;
        }
    }

    // Step 4: Build per-group expert_first_token_offset arrays
    // Each array has E+1 entries. Non-group experts contribute zero tokens,
    // so the cumulative sum stays flat at those positions.
    int64_t bf16_cumsum = 0;
    int64_t fp4_cumsum = 0;
    bf16_expert_first_token_offset[0] = 0;
    fp4_expert_first_token_offset[0] = 0;
    for (int e = 0; e < E; e++)
    {
        if (expert_precision_assignment[e] == 1) // bf16
        {
            bf16_cumsum += tokens_per_expert[e];
        }
        else // fp4
        {
            fp4_cumsum += tokens_per_expert[e];
        }
        bf16_expert_first_token_offset[e + 1] = bf16_cumsum;
        fp4_expert_first_token_offset[e + 1] = fp4_cumsum;
    }
...
// NEW: Launcher for sortExpertsByTokenCount — uses cudaLaunchKernelEx for PDL support
void sortExpertsByTokenCount(
    int64_t const* expert_first_token_offset,
    int const num_experts_per_node,
    int const num_high_precision_experts,
    int* expert_precision_assignment,
    int* bf16_expert_indices,
    int* fp4_expert_indices,
    int64_t* bf16_expert_first_token_offset,
    int64_t* fp4_expert_first_token_offset,
    cudaStream_t stream)
{
    // Shared memory: int64_t[E] for token counts + bool[E] for selection flags
    int const smem_size = num_experts_per_node * (sizeof(int64_t) + sizeof(bool));
    ...
    cudaLaunchKernelEx(&launch_config, sortExpertsByTokenCountKernel,
        expert_first_token_offset, num_experts_per_node, num_high_precision_experts,
        expert_precision_assignment, bf16_expert_indices, fp4_expert_indices,
        bf16_expert_first_token_offset, fp4_expert_first_token_offset);
}
// ============================== END Mixed-Precision Sort ==============================
    ...
    float const* router_scales, int const* permuted_row_to_unpermuted_row)
{
    ...
    // Skip experts with no tokens assigned (e.g., experts the router didn't select,
    // or experts belonging to the other precision group in mixed-precision MoE).
    // We must NOT compute pointers for these experts: the activation/output buffers
    // are sized for the group's valid token count, so pointer arithmetic with
    // num_tokens_before_expert would land at or past the buffer boundary.
    // TMA hardware may validate descriptors even for M=0 tiles, causing illegal
    // memory access faults. Setting problem shapes to {0,0,0} tells CUTLASS to
    // skip this expert entirely.
    if (gemm_m == 0)
    {
        layout_info1.shape_info.problem_shapes[expert]
            = TmaWarpSpecializedGroupedGemmInput::ProblemShape::UnderlyingProblemShape(0, 0, 0);
...
// ============================== NEW: Mixed-Precision Expand ==============================
// Dual-buffer expand for mixed-precision MoE. Routes each permuted row to either
// a bf16 output buffer (direct copy) or an fp4 output buffer (with quantization).
// The precision of each expert is determined by expert_precision_assignment[].
//
// Template parameter InputActivationsType is expected to be __nv_bfloat16 (or half).
// bf16 experts: rows are copied as-is to bf16_expanded_output.
// fp4 experts: rows are quantized to nvfp4 and written to fp4_expanded_output
//              along with per-16-element-group scaling factors in fp4_act_sf.
//
// Per-group row position: For permuted_row belonging to expert e,
//   group_row = group_expert_first_token_offset[e] + (permuted_row - global_expert_first_token_offset[e])
// This works because rows within an expert are contiguous in the permutation output.
template <class InputActivationsType>
__global__ void expandInputRowsMixedPrecisionKernel(
    InputActivationsType const* __restrict__ unpermuted_input,
    InputActivationsType* __restrict__ bf16_expanded_output,
    void* __restrict__ fp4_expanded_output,                        // __nv_fp4_e2m1 packed
    int* __restrict__ bf16_permuted_row_to_unpermuted_row,
    int* __restrict__ fp4_permuted_row_to_unpermuted_row,
    float const* __restrict__ unpermuted_scales,
    float* __restrict__ bf16_permuted_scales,
    float* __restrict__ fp4_permuted_scales,
    TmaWarpSpecializedGroupedGemmInput::ElementSF* __restrict__ fp4_act_sf,
    int const* __restrict__ permuted_row_to_unpermuted_row,
    int const* __restrict__ expert_precision_assignment,           // [E]: 0=fp4, 1=bf16
    int64_t const* __restrict__ expert_first_token_offset,        // [E+1] global
    int64_t const* __restrict__ bf16_expert_first_token_offset,   // [E+1] per-group
    int64_t const* __restrict__ fp4_expert_first_token_offset,    // [E+1] per-group
    int64_t const num_tokens,
    int64_t const hidden_size,
    int64_t const k,
    int64_t const num_experts_per_node,
    float const* __restrict__ fc1_act_global_scale,               // fp4 quantization global scale
    bool use_per_expert_act_scale)
{
    ...
    // Constants for the bf16 plain-copy path
    constexpr int64_t BF16_ELEM_PER_THREAD = 128 / sizeof_bits<InputActivationsType>::value;
    using BF16DataElem = cutlass::Array<InputActivationsType, BF16_ELEM_PER_THREAD>;

    // Constants for the fp4 quantization path
    constexpr int FP4_VecSize = TmaWarpSpecializedGroupedGemmInput::NVFP4BlockScaleVectorSize; // 16
    constexpr int64_t FP4_ELEM_PER_THREAD = CVT_ELTS_PER_THREAD; // 8
    constexpr int64_t FP4_min_k_dim_alignment = TmaWarpSpecializedGroupedGemmInput::MinKDimAlignmentNVFP4;
    int64_t const padded_hidden_size
        = TmaWarpSpecializedGroupedGemmInput::alignToSfDim(hidden_size, FP4_min_k_dim_alignment);
    ...
    for (int64_t permuted_row = blockIdx.x; permuted_row < num_valid_tokens; permuted_row += gridDim.x)
    {
        ...
        if (expert_precision_assignment[expert] == 1)
        {
            // ======= BF16 path: direct copy =======
            // Compute position within bf16 group buffer
            int64_t const global_expert_start = expert_first_token_offset[expert];
            int64_t const group_expert_start = bf16_expert_first_token_offset[expert];
            int64_t const group_row = group_expert_start + (permuted_row - global_expert_start);

            if (bf16_permuted_row_to_unpermuted_row && threadIdx.x == 0)
            {
                bf16_permuted_row_to_unpermuted_row[group_row] = static_cast<int>(unpermuted_row);
            }

            auto const* source_row_ptr
                = reinterpret_cast<BF16DataElem const*>(unpermuted_input + source_row * hidden_size);
            auto* dest_row_ptr
                = reinterpret_cast<BF16DataElem*>(bf16_expanded_output) + group_row * hidden_size / BF16_ELEM_PER_THREAD;

            int64_t const num_elems_in_col = hidden_size / BF16_ELEM_PER_THREAD;
            assert(hidden_size % BF16_ELEM_PER_THREAD == 0);
            ...
            // Write permuted scale for bf16 group
            if (bf16_permuted_scales && threadIdx.x == 0)
            {
                int64_t const source_k_idx = source_row * k + source_k_rank;
                bf16_permuted_scales[group_row] = unpermuted_scales ? unpermuted_scales[source_k_idx] : 1.0f;
            }
        }
        else
        {
            // ======= FP4 path: quantize input to nvfp4 =======
            // Compute position within fp4 group buffer
            int64_t const global_expert_start = expert_first_token_offset[expert];
            int64_t const group_expert_start = fp4_expert_first_token_offset[expert];
            int64_t const group_row = group_expert_start + (permuted_row - global_expert_start);

            if (fp4_permuted_row_to_unpermuted_row && threadIdx.x == 0)
            {
                fp4_permuted_row_to_unpermuted_row[group_row] = static_cast<int>(unpermuted_row);
            }

            // Source row pointer — read as bf16 elements in chunks of FP4_ELEM_PER_THREAD
            using FP4DataElem = cutlass::Array<InputActivationsType, FP4_ELEM_PER_THREAD>;
            auto const* source_row_ptr
                = reinterpret_cast<FP4DataElem const*>(unpermuted_input + source_row * hidden_size);
            // Destination: packed fp4 output — fp4 is sub-byte, use uint32_t for 8 fp4 elements
            auto* dest_row_ptr
                = reinterpret_cast<uint32_t*>(fp4_expanded_output) + group_row * hidden_size / FP4_ELEM_PER_THREAD;

            int64_t const num_elems_in_col = hidden_size / FP4_ELEM_PER_THREAD;
            assert(hidden_size % FP4_ELEM_PER_THREAD == 0);
            assert(hidden_size % FP4_VecSize == 0);

            // Quantization global scale for fp4
            size_t act_scale_idx = use_per_expert_act_scale ? expert : 0;
            float global_scale_val = fc1_act_global_scale ? fc1_act_global_scale[act_scale_idx] : 1.0f;
            int64_t group_expert_start_for_sf = fp4_expert_first_token_offset[expert];

            for (int64_t elem_index = start_offset; elem_index < num_elems_in_col; elem_index += stride)
            {
                auto in_vec = source_row_ptr[elem_index];
                // Quantize bf16 → nvfp4 and write scaling factors
                auto res = quantizePackedFPXValue<InputActivationsType, __nv_fp4_e2m1, FP4DataElem, FP4_VecSize>(
                    in_vec, global_scale_val, group_expert_start_for_sf, expert, group_row, elem_index,
                    padded_hidden_size, fp4_act_sf,
                    TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4);
                static_assert(
                    sizeof(res) == sizeof(*dest_row_ptr), "Quantized value must be the same size as the output");
                dest_row_ptr[elem_index] = res;
            ...
            // Pad zeros in the extra SFs along the K dimension
            size_t padding_start_offset = hidden_size / FP4_VecSize + start_offset;
            size_t padding_elems_in_col = padded_hidden_size / FP4_VecSize;
            for (int64_t elem_index = padding_start_offset; elem_index < (int64_t) padding_elems_in_col;
                 elem_index += stride)
            {
                writeSF<FP4_VecSize, FP4_VecSize>(group_expert_start_for_sf, expert, /*source_row*/ -1,
                    group_row, elem_index, padded_hidden_size, fp4_act_sf,
                    /* input_sf */ nullptr); // Pass nullptr so we write 0
            }

            // Write permuted scale for fp4 group
            if (fp4_permuted_scales && threadIdx.x == 0)
            {
                int64_t const source_k_idx = source_row * k + source_k_rank;
                fp4_permuted_scales[group_row] = unpermuted_scales ? unpermuted_scales[source_k_idx] : 1.0f;
            }
        }
    }

    // Pad zeros in the extra SFs along the N dimension for fp4 group
    // (Same pattern as existing expandInputRowsKernel N-dimension padding)
    {
        int64_t const start_pad_offset = threadIdx.x;
        int64_t const pad_stride = EXPAND_THREADS_PER_BLOCK;
        int64_t const padded_num_elems_in_col = padded_hidden_size / FP4_VecSize;
        assert(padded_hidden_size % FP4_VecSize == 0);

        constexpr int min_num_tokens_alignment = TmaWarpSpecializedGroupedGemmInput::MinNDimAlignmentNVFP4;
        static_assert((min_num_tokens_alignment & (min_num_tokens_alignment - 1)) == 0,
            "Min num tokens alignment must be a power of two");
        int64_t num_padding_tokens = min_num_tokens_alignment * num_experts_per_node;
        ...
        for (int64_t padding_token = blockIdx.x; padding_token < num_padding_tokens; padding_token += gridDim.x)
        {
            int64_t pad_expert = padding_token / min_num_tokens_alignment;
            // Only pad for fp4 experts
            if (expert_precision_assignment[pad_expert] != 0)
                continue;
            int64_t num_tokens_before_expert_in_group = fp4_expert_first_token_offset[pad_expert];
            int64_t num_tokens_after_expert_in_group = fp4_expert_first_token_offset[pad_expert + 1];
            int64_t tokens_to_expert = num_tokens_after_expert_in_group - num_tokens_before_expert_in_group;
            int64_t padding_to_expert
                = TmaWarpSpecializedGroupedGemmInput::alignToSfDim(tokens_to_expert, min_num_tokens_alignment)
                - tokens_to_expert;
            int64_t expert_pad_idx = padding_token % min_num_tokens_alignment;
            if (expert_pad_idx < padding_to_expert)
            {
                for (int64_t elem_index = start_pad_offset; elem_index < padded_num_elems_in_col;
                     elem_index += pad_stride)
                {
                    writeSF<FP4_VecSize, FP4_VecSize>(num_tokens_before_expert_in_group, pad_expert,
                        /*source_row*/ -1, num_tokens_after_expert_in_group + expert_pad_idx,
                        elem_index, padded_hidden_size, fp4_act_sf,
                        /* input_sf */ nullptr); // Pass nullptr so we write 0
                }
            }
        }
...
// NEW: Launcher for expandInputRowsMixedPrecisionKernel
template <class InputActivationsType>
void expandInputRowsMixedPrecisionKernelLauncher(
    InputActivationsType const* unpermuted_input,
    void* bf16_expanded_output,
    void* fp4_expanded_output,
    ...
    TmaWarpSpecializedGroupedGemmInput::ElementSF* fp4_act_sf,
    int const* permuted_row_to_unpermuted_row,
    int const* expert_precision_assignment,
    int64_t const* expert_first_token_offset,
    int64_t const* bf16_expert_first_token_offset,
    int64_t const* fp4_expert_first_token_offset,
    ...
    cudaStream_t stream)
{
    ...
    auto func = &expandInputRowsMixedPrecisionKernel<InputActivationsType>;

    static int32_t const smCount = tensorrt_llm::common::getMultiProcessorCount();
    int32_t const maxBlocksPerSM = tensorrt_llm::common::getMaxActiveBlocksPerSM(func, EXPAND_THREADS_PER_BLOCK, 0);
    int32_t const blocks
        = std::min(smCount * maxBlocksPerSM,
            static_cast<int32_t>(std::max(num_rows * experts_per_token, num_padding_tokens)));
    int32_t const threads = EXPAND_THREADS_PER_BLOCK;
    ...
    cudaLaunchKernelEx(&launch_config, func,
        unpermuted_input,
        reinterpret_cast<InputActivationsType*>(bf16_expanded_output),
        fp4_expanded_output,
        bf16_permuted_row_to_unpermuted_row,
        fp4_permuted_row_to_unpermuted_row,
        unpermuted_scales,
        bf16_permuted_scales,
        fp4_permuted_scales,
        fp4_act_sf,
        ...
        num_rows,
        hidden_size,
        static_cast<int64_t>(experts_per_token),
        static_cast<int64_t>(num_experts_per_node),
        fc1_act_global_scale,
        use_per_expert_act_scale);
#else
    TLLM_THROW("expandInputRowsMixedPrecisionKernelLauncher requires ENABLE_FP4");
#endif // ENABLE_FP4
}

// Explicit template instantiations for expandInputRowsMixedPrecisionKernelLauncher
#ifdef ENABLE_BF16
template void expandInputRowsMixedPrecisionKernelLauncher<__nv_bfloat16>(
    __nv_bfloat16 const* unpermuted_input, void* bf16_expanded_output, void* fp4_expanded_output,
    int* bf16_permuted_row_to_unpermuted_row, int* fp4_permuted_row_to_unpermuted_row,
    float const* unpermuted_scales, float* bf16_permuted_scales, float* fp4_permuted_scales,
    TmaWarpSpecializedGroupedGemmInput::ElementSF* fp4_act_sf,
    int const* permuted_row_to_unpermuted_row, int const* expert_precision_assignment,
    int64_t const* expert_first_token_offset, int64_t const* bf16_expert_first_token_offset,
    int64_t const* fp4_expert_first_token_offset, int64_t const num_rows, int64_t const hidden_size,
    int const experts_per_token, int const num_experts_per_node, float const* fc1_act_global_scale,
    bool use_per_expert_act_scale, cudaStream_t stream);
#endif
template void expandInputRowsMixedPrecisionKernelLauncher<half>(
    half const* unpermuted_input, void* bf16_expanded_output, void* fp4_expanded_output,
    int* bf16_permuted_row_to_unpermuted_row, int* fp4_permuted_row_to_unpermuted_row,
    float const* unpermuted_scales, float* bf16_permuted_scales, float* fp4_permuted_scales,
    TmaWarpSpecializedGroupedGemmInput::ElementSF* fp4_act_sf,
    int const* permuted_row_to_unpermuted_row, int const* expert_precision_assignment,
...
// NEW: Fused mixed-precision activation kernel (Step 4 of fusedMixPrecisionMoE plan).
// Processes contiguous [bf16_gemm1_out | fp4_gemm1_out] buffer in a single kernel launch.
// Rows [0, boundary_index): SwiGLU -> bf16 activation output
// Rows [boundary_index, total_rows): SwiGLU + NVFP4 quantize -> fp4 output + scaling factors
// Both paths use CVT_ELTS_PER_THREAD (= 8) elements per thread, matching bf16's 128/sizeof_bits<bf16>.
template <class GemmOutputType, class ScaleBiasType, class ActFn, int kProcessRows>
__global__ void doMixedPrecisionActivationKernel(
    __nv_bfloat16* activation_output_bf16,
    __nv_fp4_e2m1* activation_output_fp4,
    TmaWarpSpecializedGroupedGemmInput::ElementSF* fp4_act_sf_flat,
    GemmOutputType const* gemm1_output,
    ...
    int64_t total_rows,
    float const* bf16_fp8_quant,
    float const* fp4_fp8_quant,
    float const* fp4_fc2_act_global_scale,
    bool fp4_use_per_expert_act_scale,
    ActivationParams activation_params,
    GemmOutputType const* prequant_scale_fp4)
{
#ifdef ENABLE_FP4
    constexpr bool IsGated = ActFn::IS_GLU;
    size_t const gated_size_mul = IsGated ? 2 : 1;
    size_t const gated_off = IsGated ? inter_size : 0;

    // Both bf16 and NVFP4 paths use 8 elements per thread
    ...
    // 2D grid: blockIdx.x for token rows (groups of kProcessRows), blockIdx.y for columns
    int64_t const row_offset = blockIdx.x * kProcessRows;
    int64_t const col_offset = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    // K-dimension SF padding range (applies only to fp4 rows)
    ...
    using BiasElem = cutlass::Array<ScaleBiasType, ACTIVATION_ELEM_PER_THREAD>;
    using ComputeElem = cutlass::Array<float, ACTIVATION_ELEM_PER_THREAD>;
    using BF16OutputElem = cutlass::Array<__nv_bfloat16, ACTIVATION_ELEM_PER_THREAD>;
    using FP4OutputElem = uint32_t; // 8 fp4 values packed into 32 bits

    auto main_loop = [&](auto has_bias)
    {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
        cudaGridDependencySynchronize();
#endif
...
#pragma unroll
        for (int i = 0; i < kProcessRows; ++i)
        {
            ...
            bool const is_bf16_row = (token < boundary_index);

            // Input from contiguous GLU buffer
            size_t const gemm_result_offset = token * inter_size * gated_size_mul;
            auto gemm_result_vec = reinterpret_cast<GemmResultElem const*>(gemm1_output + gemm_result_offset);

            // Expert lookup: use group-appropriate offset array and token index
            int64_t const* expert_offsets
                = is_bf16_row ? bf16_expert_first_token_offset : fp4_expert_first_token_offset;
            int64_t const token_in_group = is_bf16_row ? token : (token - boundary_index);

            int64_t expert = 0;
            bool const need_expert = bias_ptr || !is_bf16_row || activation_params.swiglu_alpha
                || activation_params.swiglu_beta || activation_params.swiglu_limit;
            if (need_expert)
            {
                expert = findTotalEltsLessThanTarget(
                             expert_offsets, num_experts_per_node, token_in_group + 1)
                    - 1;
            }

            float gate_alpha = activation_params.swiglu_alpha ? activation_params.swiglu_alpha[expert] : 1.0f;
            float gate_beta = activation_params.swiglu_beta ? activation_params.swiglu_beta[expert] : 0.0f;
            float gate_limit = activation_params.swiglu_limit ? activation_params.swiglu_limit[expert]
                                                              : std::numeric_limits<float>::infinity();
            ...
            // Precompute bias pointer for this row
            [[maybe_unused]] size_t const bias_offset
                = has_bias_v ? (bias_is_broadcast ? expert * inter_size * gated_size_mul : gemm_result_offset) : 0;
            [[maybe_unused]] auto bias_ptr_vec
                = has_bias_v ? reinterpret_cast<BiasElem const*>(bias_ptr + bias_offset) : nullptr;

            // Load gated value from GLU buffer
            auto fc1_value = arrayConvert<GemmResultElem, ComputeElem>(
                loadVec(&gemm_result_vec[elem_index + gated_off_vec]));
            if constexpr (has_bias_v)
            {
                fc1_value = fc1_value
                    + arrayConvert<BiasElem, ComputeElem>(loadVec(&bias_ptr_vec[elem_index + gated_off_vec]));
            ...
            // Apply activation (SwiGLU or other gated activation)
            auto gate_act = [&]()
            {
                if constexpr (IsGated)
                {
            ...
            if (is_bf16_row)
            {
                // ---- BF16 path: optional fp8 quant scale, write bf16 ----
                float const quant_scale = bf16_fp8_quant ? bf16_fp8_quant[0] : 1.f;
                auto post_act_val = gate_act * quant_scale;

                size_t const output_offset = token * inter_size;
                auto output_vec = reinterpret_cast<BF16OutputElem*>(activation_output_bf16 + output_offset);
                storeVec(&output_vec[elem_index], arrayConvert<ComputeElem, BF16OutputElem>(post_act_val));
            }
            else
            {
                // ---- FP4 path: quant scale + NVFP4 quantize, write fp4 + SF ----
                size_t const act_scale_idx = fp4_use_per_expert_act_scale ? expert : 0;
                float const quant_scale = fp4_fp8_quant ? fp4_fp8_quant[act_scale_idx] : 1.f;
                auto post_act_val = gate_act * quant_scale;

                // Apply per-expert prequant scale if provided
                if (prequant_scale_fp4)
                {
                    auto prequant_vec
                        = reinterpret_cast<GemmResultElem const*>(prequant_scale_fp4 + expert * inter_size);
                    post_act_val = post_act_val
                        * arrayConvert<GemmResultElem, ComputeElem>(loadVec(&prequant_vec[elem_index]));
                }

                float const global_scale_val
                    = fp4_fc2_act_global_scale ? fp4_fc2_act_global_scale[act_scale_idx] : 1.0f;
                int64_t const num_tokens_before_expert = fp4_expert_first_token_offset[expert];
                int64_t const fp4_token = token - boundary_index;

                size_t const output_offset = fp4_token * inter_size;
                auto output_vec
                    = reinterpret_cast<FP4OutputElem*>(safe_inc_ptr(activation_output_fp4, output_offset));

                auto res = quantizePackedFPXValue<GemmOutputType, __nv_fp4_e2m1, ComputeElem, VecSize>(
                    post_act_val, global_scale_val, num_tokens_before_expert, expert, fp4_token, elem_index,
                    inter_size, fp4_act_sf_flat,
                    TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4);
                static_assert(
                    sizeof(res) == sizeof(*output_vec), "Quantized value must be the same size as the output");
                output_vec[elem_index] = res;

                // K-dimension SF padding for fp4
                if (do_k_padding)
                {
                    writeSF<VecSize, VecSize>(num_tokens_before_expert, expert, /*source_row*/ -1, fp4_token,
                        col_offset, padded_inter_size, fp4_act_sf_flat, nullptr);
                }
            }
        }
        ...
        // N-dimension SF padding for fp4 experts (blocks beyond the data range)
        {
            ...
            int64_t const num_token_blocks = (total_rows + kProcessRows - 1) / kProcessRows;
            if (blockIdx.x < num_token_blocks)
                return;
...
#pragma unroll
            for (int i = 0; i < kProcessRows; ++i)
            {
                ...
                int64_t const pad_expert = padding_token / min_num_tokens_alignment;
                int64_t const num_tokens_before_expert = fp4_expert_first_token_offset[pad_expert];
                int64_t const num_tokens_after_expert = fp4_expert_first_token_offset[pad_expert + 1];
                int64_t const tokens_to_expert = num_tokens_after_expert - num_tokens_before_expert;
                int64_t const padding_to_expert
                    = TmaWarpSpecializedGroupedGemmInput::alignToSfDim(
                          tokens_to_expert, min_num_tokens_alignment)
                    - tokens_to_expert;
                int64_t const expert_pad_idx = padding_token % min_num_tokens_alignment;
                if (expert_pad_idx < padding_to_expert)
                {
                    writeSF<VecSize, VecSize>(num_tokens_before_expert, pad_expert, /*source_row*/ -1,
                        num_tokens_after_expert + expert_pad_idx, sf_col_offset, padded_inter_size,
                        fp4_act_sf_flat, nullptr);
                }
            }
        }
    }; // end main_loop
    ...
    if (bias_ptr)
    {
        main_loop(std::true_type{});
    }
    else
    {
        main_loop(std::false_type{});
    }
#endif // ENABLE_FP4
}

// Host launcher for doMixedPrecisionActivationKernel
    ...
    GemmOutputType const* prequant_scale_fp4, cudaStream_t stream)
{
    ...
    auto doLaunch = [&](auto num_rows_per_cta)
    {
        constexpr int num_rows_per_cta_v = num_rows_per_cta.value;

        // Select activation functor based on activation type
        using Swiglu = GLUAdaptor<cutlass::epilogue::thread::SiLu>;
        using Geglu = GLUAdaptor<cutlass::epilogue::thread::GELU>;
        using SwigluBias_t = SwigluBiasAdaptor;

        auto fn = (activation_type.activation_type == ActivationType::Swiglu)
            ? &doMixedPrecisionActivationKernel<GemmOutputType, ScaleBiasType, Swiglu, num_rows_per_cta_v>
            : activation_type.activation_type == ActivationType::Geglu
            ? &doMixedPrecisionActivationKernel<GemmOutputType, ScaleBiasType, Geglu, num_rows_per_cta_v>
            : activation_type.activation_type == ActivationType::SwigluBias
            ? &doMixedPrecisionActivationKernel<GemmOutputType, ScaleBiasType, SwigluBias_t, num_rows_per_cta_v>
            : nullptr;
        TLLM_CHECK_WITH_INFO(fn != nullptr, "doMixedPrecisionActivation: unsupported activation type");

        // Extra blocks for N-dimension SF padding (fp4 only)
        int64_t const num_padding_tokens
            = TmaWarpSpecializedGroupedGemmInput::MinNDimAlignmentNVFP4 * num_experts_per_node;
        int64_t const num_token_blocks = (total_rows + num_rows_per_cta_v - 1) / num_rows_per_cta_v;
        int64_t const num_padding_blocks = (num_padding_tokens + num_rows_per_cta_v - 1) / num_rows_per_cta_v;
        int32_t const grid_x = static_cast<int32_t>(num_token_blocks + num_padding_blocks);
        int32_t const grid_y = static_cast<int32_t>(
            (num_elems_in_col + ACTIVATION_THREADS_PER_BLOCK - 1) / ACTIVATION_THREADS_PER_BLOCK);
        ...
        cudaLaunchKernelEx(&config, fn, activation_output_bf16, activation_output_fp4, fp4_act_sf_flat,
            gemm1_output, bias_ptr, bias_is_broadcast, bf16_expert_first_token_offset,
            fp4_expert_first_token_offset, num_experts_per_node, inter_size, boundary_index, total_rows,
            bf16_fp8_quant, fp4_fp8_quant, fp4_fc2_act_global_scale, fp4_use_per_expert_act_scale,
            activation_type, prequant_scale_fp4);
    };

    // Choose rows per CTA based on workload (same heuristic as doActivation)
    if (num_elems_in_col * total_rows < 256 * 256)
    {
        doLaunch(std::integral_constant<int, 1>());
    }
    else if (num_elems_in_col * total_rows < 256 * 512)
    {
        doLaunch(std::integral_constant<int, 2>());
    }
    else
    {
        doLaunch(std::integral_constant<int, 4>());
    }
#endif // ENABLE_FP4
}

template <typename T>
__global__ void accumulateMixedPrecisionOutputKernel(T* output, T const* partial_output, int64_t num_elements)
{
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
    cudaGridDependencySynchronize();
#endif
    ...
    for (int64_t idx = tid; idx < num_elements; idx += stride)
    {
        float const value = static_cast<float>(output[idx]) + static_cast<float>(partial_output[idx]);
...
void accumulateMixedPrecisionOutput(T* output, T const* partial_output, int64_t num_elements, cudaStream_t stream)
{
    ...
    int32_t const sm_count = tensorrt_llm::common::getMultiProcessorCount();
    int64_t const max_blocks = static_cast<int64_t>(sm_count) * 4;
    int32_t const blocks = static_cast<int32_t>(std::max<int64_t>(1,
        std::min<int64_t>(tensorrt_llm::common::ceilDiv(num_elements, static_cast<int64_t>(threads)), max_blocks)));
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::
    getWorkspaceDeviceBufferSizesMixedPrecision(int64_t const num_rows, int64_t const hidden_size,
        int64_t const inter_size, int const num_experts_per_node, int const experts_per_token,
        ActivationType activation_type)
{
    size_t const expanded_rows = num_rows * experts_per_token;
    size_t const expanded_elems = expanded_rows * hidden_size;
    size_t const inter_elems = expanded_rows * inter_size;
    size_t const gated_inter_elems = isGatedActivation(activation_type) ? inter_elems * 2 : inter_elems;

    int64_t const num_tokens_per_block = computeNumTokensPerBlock(num_rows, num_experts_per_node);
    int64_t const num_blocks_per_seq = tensorrt_llm::common::ceilDiv(num_rows, num_tokens_per_block);

    size_t const permuted_row_to_unpermuted_row_size = expanded_rows * sizeof(int);
    size_t const permuted_token_selected_experts_size = expanded_rows * sizeof(int);
    ...
    size_t const expert_precision_assignment_size = num_experts_per_node * sizeof(int);
    size_t const bf16_expert_indices_size = num_experts_per_node * sizeof(int);
    size_t const fp4_expert_indices_size = num_experts_per_node * sizeof(int);
    size_t const bf16_expert_first_token_offset_size = (num_experts_per_node + 1) * sizeof(int64_t);
    size_t const fp4_expert_first_token_offset_size = (num_experts_per_node + 1) * sizeof(int64_t);
    size_t const mixed_prec_state_size = sizeof(MixedPrecisionMoeState);

    size_t const bf16_permuted_row_to_unpermuted_row_size = expanded_rows * sizeof(int);
    size_t const fp4_permuted_row_to_unpermuted_row_size = expanded_rows * sizeof(int);

    size_t const bf16_expanded_data_size = expanded_elems * sizeof(T);
    size_t const fp4_expanded_data_size = tensorrt_llm::common::ceilDiv(expanded_elems, static_cast<size_t>(2));

    auto const act_sf_rows = std::min(expanded_rows, static_cast<size_t>(num_rows * num_experts_per_node));
    size_t const fp4_expanded_sf_size = getOffsetActivationSF(num_experts_per_node, act_sf_rows, hidden_size,
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4)
        * sizeof(TmaWarpSpecializedGroupedGemmInput::NVFP4ElementSF);
    ...
    size_t const fp4_activation_output_size = tensorrt_llm::common::ceilDiv(inter_elems, static_cast<size_t>(2));
    size_t const fp4_gemm2_act_sf_size = getOffsetActivationSF(num_experts_per_node, act_sf_rows, inter_size,
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4)
        * sizeof(TmaWarpSpecializedGroupedGemmInput::NVFP4ElementSF);
    ...
    size_t const fp4_permuted_scales_size = expanded_rows * sizeof(float);
...
#define ADD(name) ADD_NAME(name, name##_size)
    ...
    ADD(expert_precision_assignment);
    ADD(bf16_expert_indices);
    ADD(fp4_expert_indices);
    ADD(bf16_expert_first_token_offset);
    ADD(fp4_expert_first_token_offset);
    ADD(mixed_prec_state);

    ADD(bf16_permuted_row_to_unpermuted_row);
    ADD(fp4_permuted_row_to_unpermuted_row);

    ADD(bf16_expanded_data);
    ...
    ADD(fp4_permuted_scales);

    ADD_NAME(alpha_scale_ptr_array_fc1, alpha_scale_ptr_array_size);
    ADD_NAME(alpha_scale_ptr_array_fc2, alpha_scale_ptr_array_size);

    ADD_NAME(mixed_tma_ws_gemm1_workspace, bf16_tma_ws_size);
    ADD_NAME(mixed_tma_ws_gemm2_workspace, bf16_tma_ws_size);
    ADD_NAME(mixed_fp4_tma_ws_gemm1_workspace, fp4_tma_ws_size);
    ADD_NAME(mixed_fp4_tma_ws_gemm2_workspace, fp4_tma_ws_size);
    ADD_NAME(mixed_gemm_workspace, mixed_gemm_workspace_size);
...
template <class T, class WeightType, class OutputType, class InputType, class BackBoneType, class Enable>
size_t CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::getMixedPrecisionWorkspaceSize(
    ...
    int const experts_per_token, ActivationType activation_type, MOEParallelismConfig parallelism_config)
{
    ...
    auto sizes_map = getWorkspaceDeviceBufferSizesMixedPrecision(
    ...
    TLLM_LOG_TRACE("Mixture Of Experts Mixed Precision requires workspace of %2f MiB", size / 1024.f / 1024.f);
    return size;
}
...
// ======================================================================================
// scatterAddMixedPrecFinalizeKernel: Non-TMA-WS finalize for mixed-precision BF16 GEMM2.
//
// On SM120 (Blackwell), the BF16 MoE GEMM runner does NOT support TMA warp-specialized
// kernels (only FP4×FP4 and FP8×FP4 are valid SM120 TMA WS specializations).
// This means the BF16 GEMM2 output is at GROUP-LOCAL permuted indices and cannot use
// the TMA WS fused finalize epilogue. Instead, this kernel performs the finalize step:
//   (1) For each group-local permuted row, look up the original token index,
//   (2) Scale by the router weight,
//   (3) Add expert bias (if present),
//   (4) Atomically add the contribution to the shared final_output buffer.
//
// Requires SM90+ for atomicAdd on __nv_bfloat16. Guarded by __CUDA_ARCH__ >= 900.
// ======================================================================================
template <typename OutputType, typename GemmOutputType, typename ScaleBiasType>
__global__ void scatterAddMixedPrecFinalizeKernel(
    GemmOutputType const* __restrict__ group_gemm2_output,
    OutputType* __restrict__ final_output,
    ScaleBiasType const* __restrict__ bias,
    float const* __restrict__ group_permuted_scales,
    int const* __restrict__ group_permuted_row_to_unpermuted,
    int64_t const* __restrict__ group_expert_first_token_offset,
    int64_t group_valid_tokens,
    int64_t num_rows,
    int64_t hidden_size,
...
{
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
    int64_t const group_row = blockIdx.x;
    if (group_row >= group_valid_tokens)
        return;

    // Map group-local permuted row back to original token index.
    // group_permuted_row_to_unpermuted stores: unpermuted_idx = token_row * experts_per_token + expert_slot
    // We need just the token row: token_row = unpermuted_idx / experts_per_token.
    // But we don't have experts_per_token here — instead, unpermuted_idx % num_rows gives the token row
    // because unpermuted indices are laid out as [token0_expert0, token1_expert0, ..., tokenN_expert0,
    // token0_expert1, ...], i.e., the row index into the expanded (num_rows * experts_per_token) table.
    int64_t const unpermuted_idx = group_permuted_row_to_unpermuted[group_row];
    int64_t const token_row = unpermuted_idx % num_rows;

    float const scale = group_permuted_scales[group_row];

    auto const* input_row = group_gemm2_output + group_row * hidden_size;
    auto* output_row = final_output + token_row * unpadded_hidden_size;

    // Determine which expert this row belongs to, for bias lookup.
    ScaleBiasType const* bias_row = nullptr;
    if (bias)
    {
        // Binary search would be cleaner, but num_experts_per_node is small (typically 8-64).
        int expert_id = num_experts_per_node - 1;
        for (int e = 0; e < num_experts_per_node; ++e)
        {
            if (group_row < group_expert_first_token_offset[e + 1])
            {
                expert_id = e;
                break;
            }
        }
        bias_row = bias + expert_id * hidden_size;
    }

    // Each thread handles a slice of columns. Scale, add bias, and atomically accumulate.
    for (int64_t col = threadIdx.x; col < unpadded_hidden_size; col += blockDim.x)
    {
        float val = static_cast<float>(input_row[col]) * scale;
        if (bias_row)
        {
            val += static_cast<float>(bias_row[col]);
        }
        atomicAdd(output_row + col, static_cast<OutputType>(__float2bfloat16_rn(val)));
    }
#else
    // This kernel is only valid on SM90+. On older architectures, mixed-precision MoE
    // with non-TMA-WS BF16 GEMM2 is not supported.
    (void)group_gemm2_output;
    (void)final_output;
    (void)bias;
    (void)group_permuted_scales;
    (void)group_permuted_row_to_unpermuted;
    (void)group_expert_first_token_offset;
    (void)group_valid_tokens;
    (void)num_rows;
    (void)hidden_size;
    (void)unpadded_hidden_size;
    (void)num_experts_per_node;
#endif
}

template <typename OutputType, typename GemmOutputType, typename ScaleBiasType>
void scatterAddMixedPrecFinalizeLauncher(
    GemmOutputType const* group_gemm2_output,
    OutputType* final_output,
    ScaleBiasType const* bias,
    float const* group_permuted_scales,
    int const* group_permuted_row_to_unpermuted,
    int64_t const* group_expert_first_token_offset,
    int64_t group_valid_tokens,
    int64_t num_rows,
    int64_t hidden_size,
    int64_t unpadded_hidden_size,
    ...
    cudaStream_t stream)
{
    if (group_valid_tokens <= 0)
        return;
    int const threads = FINALIZE_THREADS_PER_BLOCK;
    int const blocks = static_cast<int>(group_valid_tokens);
    scatterAddMixedPrecFinalizeKernel<<<blocks, threads, 0, stream>>>(
        group_gemm2_output, final_output, bias,
        group_permuted_scales, group_permuted_row_to_unpermuted,
        group_expert_first_token_offset, group_valid_tokens,
        num_rows, hidden_size, unpadded_hidden_size, num_experts_per_node);
}
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::runMixedPrecisionMoe(
    ...
    void const* fc1_expert_weights_bf16_void, void const* fc1_expert_weights_fp4_void,
    void const* fc1_expert_biases_void, ActivationParams fc1_activation_type,
    void const* fc2_expert_weights_bf16_void, void const* fc2_expert_weights_fp4_void,
...
{
    int const num_experts_per_node = full_num_experts / parallelism_config.ep_size;
    bool const all_fp4 = (num_high_precision_experts == 0);

    // --- Dispatch: if all fp4, delegate to the fp4-typed runner via runMoe ---
    if (all_fp4 && fp4_runner)
    {
        // DEBUG: print inputs to all-fp4 runMoe
        {
            TLLM_LOG_WARNING("[MixedPrecMoE] all_fp4 runMoe inputs:");
            TLLM_LOG_WARNING("  num_rows=%ld, num_valid_rows=%ld, hidden_size=%ld, inter_size=%ld",
                (long)num_rows, (long)num_valid_rows, (long)hidden_size, (long)inter_size);
            TLLM_LOG_WARNING("  full_num_experts=%d, experts_per_token=%ld, unpadded_hidden_size=%ld",
                full_num_experts, (long)experts_per_token, (long)unpadded_hidden_size);
            TLLM_LOG_WARNING("  input_activations=%p, fc1_weights_fp4=%p, fc2_weights_fp4=%p",
                input_activations_void, fc1_expert_weights_fp4_void, fc2_expert_weights_fp4_void);
            TLLM_LOG_WARNING("  fc1_biases=%p, fc2_biases=%p, final_output=%p",
                fc1_expert_biases_void, fc2_expert_biases_void, final_output_void);

            // Print first few elements of input activations (as bf16)
            int const print_n = 8;
            {
                std::vector<__nv_bfloat16> tmp(print_n);
                cudaMemcpy(tmp.data(), input_activations_void, print_n * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost);
                std::string s = "  input[0..7]: ";
                for (int i = 0; i < print_n; i++) {
                    if (i > 0) s += ", ";
                    s += std::to_string(__bfloat162float(tmp[i]));
                }
                TLLM_LOG_WARNING("%s", s.c_str());
            }

            // Print first few elements of fp4 weights (as raw int64)
            {
                std::vector<int64_t> tmp(print_n);
                cudaMemcpy(tmp.data(), fc1_expert_weights_fp4_void, print_n * sizeof(int64_t), cudaMemcpyDeviceToHost);
                std::string s = "  fc1_fp4_weights[0..7] (int64): ";
                for (int i = 0; i < print_n; i++) {
                    if (i > 0) s += ", ";
            ...
            {
                int const total = std::min((int)(num_rows * experts_per_token), print_n);
                std::vector<int> tmp(total);
                cudaMemcpy(tmp.data(), token_selected_experts, total * sizeof(int), cudaMemcpyDeviceToHost);
                std::string s = "  token_selected_experts[0.." + std::to_string(total - 1) + "]: ";
            ...
            // Print token_final_scales
            {
                ...
                cudaMemcpy(tmp.data(), token_final_scales, total * sizeof(float), cudaMemcpyDeviceToHost);
                std::string s = "  token_final_scales[0.." + std::to_string(total - 1) + "]: ";
                for (int i = 0; i < total; i++) {
                    if (i > 0) s += ", ";
                    s += std::to_string(tmp[i]);
                }
        ...
        LoraParams dummy_lora{};
        MoeMinLatencyParams dummy_min_latency{};
        fp4_runner->runMoe(
            input_activations_void, input_sf_void, swizzled_input_sf,
            token_selected_experts, token_final_scales,
            fc1_expert_weights_fp4_void,
            fc1_expert_biases_void, fc1_activation_type,
            fc2_expert_weights_fp4_void,
            fc2_expert_biases_void,
            fp4_quant_params,
            num_rows, num_valid_rows, hidden_size, unpadded_hidden_size, inter_size,
            full_num_experts, experts_per_token,
            workspace_ptr, final_output_void, unpermuted_row_to_permuted_row,
            parallelism_config, enable_alltoall,
            /*use_lora=*/false, dummy_lora,
            /*use_deepseek_fp8_block_scale=*/false,
            /*min_latency_mode=*/false, dummy_min_latency,
            stream);
        auto* final_output = static_cast<OutputType*>(final_output_void);

        // DEBUG: print first few elements of final_output after all-fp4 runMoe
        {
            sync_check_cuda_error(stream);
            int const print_elems = std::min((int)(num_rows * hidden_size), 16);
            std::vector<float> host_buf(print_elems);
            if constexpr (std::is_same_v<OutputType, __nv_bfloat16>)
            {
                std::vector<__nv_bfloat16> tmp(print_elems);
                cudaMemcpy(tmp.data(), final_output, print_elems * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost);
                for (int i = 0; i < print_elems; i++) host_buf[i] = __bfloat162float(tmp[i]);
            }
            else
            {
                cudaMemcpy(host_buf.data(), final_output, print_elems * sizeof(float), cudaMemcpyDeviceToHost);
            }
            std::string s = "[MixedPrecMoE] all_fp4 final_output[0.." + std::to_string(print_elems - 1) + "]: ";
    ...
    // --- Normal path: this runner handles the weights via its template types ---
    auto const* input_activations = static_cast<InputType const*>(input_activations_void);
    auto const* input_sf = input_sf_void
        ? reinterpret_cast<TmaWarpSpecializedGroupedGemmInput::ElementSF const*>(input_sf_void)
        : nullptr;
    auto const* fc1_expert_weights = static_cast<WeightType const*>(fc1_expert_weights_bf16_void);
    auto const* fc1_expert_biases = reinterpret_cast<ScaleBiasType const*>(fc1_expert_biases_void);
    auto const* fc2_expert_weights = static_cast<WeightType const*>(fc2_expert_weights_bf16_void);
    auto const* fc2_expert_biases = reinterpret_cast<ScaleBiasType const*>(fc2_expert_biases_void);
    auto* final_output = static_cast<OutputType*>(final_output_void);
    float const* token_topk_unpermuted_scales = token_final_scales;
    auto const& quant_params = bf16_quant_params;
    ...
    // --- Workspace setup (no lora, no deepseek block scale, no min latency, no awq) ---
    configureWsPtrs(workspace_ptr, num_rows, hidden_size, inter_size, num_experts_per_node, experts_per_token,
        fc1_activation_type.activation_type, parallelism_config, /*use_lora=*/false,
        /*use_deepseek_fp8_block_scale=*/false, /*min_latency_mode=*/false, /*use_awq=*/false);
    ...
    // --- Expand input rows ---
    bool use_per_expert_act_scale = use_fp4 ? quant_params.fp4.fc1.use_per_expert_act_scale : false;
    expandInputRowsKernelLauncher(input_activations, reinterpret_cast<T*>(permuted_data_),
        token_topk_unpermuted_scales, permuted_token_final_scales_, permuted_row_to_unpermuted_row_, num_rows,
        hidden_size, experts_per_token, num_experts_per_node, quant_params, use_per_expert_act_scale,
        expert_first_token_offset_, fc1_fp4_act_scale_, input_sf, swizzled_input_sf,
        /*prequant_act_scale=*/nullptr, stream);
    sync_check_cuda_error(stream);
    ...
    auto [gemm1_tma_ws_input, gemm2_tma_ws_input]
        = setupTmaWarpSpecializedInputs(num_rows, expanded_num_rows, fc1_activation_type, hidden_size,
            unpadded_hidden_size, inter_size, num_experts_per_node, input_activations_void, input_sf, final_output,
            fc1_expert_weights, fc2_expert_weights, quant_params, fc1_expert_biases, fc2_expert_biases,
            /*min_latency_mode=*/false, dummy_min_latency, /*use_lora=*/false, start_expert,
            parallelism_config, stream);

    // --- GEMM1 (FC1: hidden_size -> inter_size, with fused activation) ---
    Self::gemm1(moe_gemm_runner_, /*blockscale_gemm_runner=*/nullptr,
        reinterpret_cast<T const*>(permuted_data_), fc1_result_, glu_inter_result_,
        expert_first_token_offset_, gemm1_tma_ws_input,
        fc1_expert_weights, fc1_expert_biases, num_valid_tokens_ptr,
        /*fc1_int_scales=*/nullptr, /*fc1_fp8_dequant=*/nullptr, /*fc2_fp8_quant=*/nullptr,
        fc1_fp4_act_scale_, fc2_fp4_act_scale_, quant_params,
        num_rows, expanded_num_rows, expected_tokens_per_expert,
        hidden_size, inter_size, num_experts_per_node,
        fc1_activation_type, alpha_scale_ptr_array_fc1_,
        /*bias_is_broadcast=*/true, stream, *gemm1_config_,
        /*min_latency_mode=*/false, /*active_expert_count=*/nullptr, /*active_expert_ids=*/nullptr,
        /*fc2_prequant_scale=*/nullptr);
    sync_check_cuda_error(stream);

    // --- GEMM2 (FC2: inter_size -> hidden_size, with finalize) ---
    Self::gemm2(moe_gemm_runner_, /*blockscale_gemm_runner=*/nullptr,
        fc1_result_, fc2_result_, final_output,
        expert_first_token_offset_, gemm2_tma_ws_input,
        fc2_expert_weights, fc2_expert_biases,
        /*fc2_int_scales=*/nullptr, /*fc2_fp8_dequant=*/nullptr,
        fc2_fp4_act_scale_, quant_params, token_topk_unpermuted_scales,
        permuted_token_final_scales_, unpermuted_row_to_permuted_row, permuted_row_to_unpermuted_row_,
        token_selected_experts, num_valid_tokens_ptr,
        num_rows, expanded_num_rows, expected_tokens_per_expert,
        hidden_size, unpadded_hidden_size, inter_size,
        num_experts_per_node, experts_per_token,
        alpha_scale_ptr_array_fc2_,
        /*use_lora=*/false, /*lora_fc2_result=*/nullptr,
        stream, parallelism_config, enable_alltoall,
        *gemm2_config_, /*min_latency_mode=*/false,
        /*active_expert_count=*/nullptr, /*active_expert_ids=*/nullptr);
    sync_check_cuda_error(stream);
}
