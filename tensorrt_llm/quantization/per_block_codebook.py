...
class PerBlockAQLMWithEntropyCoding(PerBlockCodebookBase):
    """
    Phase 5: AQLM with Entropy Coding.
    
    Extends Phase 4 (AQLM) with Huffman coding of indices for additional compression.
    ...
    def __init__(self,
                 ...
                 dtype: torch.dtype = torch.float32):
        """
        Initialize AQLM with entropy coding.
    ...
    def _build_huffman_tree(self, indices: torch.Tensor) -> Dict:
        """
        Build Huffman tree for indices.
        
        Args:
            indices: Index tensor of shape (n,)
            
        Returns:
            Dict with Huffman codes and tree
        """
        import heapq
        ...
        codes = {val: code for val, code in huffman_code}
        
        return codes
    
    def _encode_indices_huffman(self, indices: torch.Tensor, codes: Dict) -> Tuple[str, Dict]:
        """
        ...
        """
        flat_indices = indices.cpu().numpy().flatten()
        encoded = ''.join(codes[int(idx)] for idx in flat_indices)
        ...
        return encoded_bytes, {'padding': padding, 'codes': codes}
    
    def _decode_indices_huffman(self, encoded_bytes: bytes, codes: Dict, 
                                original_shape: Tuple) -> torch.Tensor:
        """
        Decode indices from Huffman codes.
        
        Args:
            encoded_bytes: Encoded byte string
            codes: Huffman code dictionary
            original_shape: Original shape of indices
            
        Returns:
            Decoded indices tensor
        """
        # Convert bytes to binary string
        binary = ''.join(format(byte, '08b') for byte in encoded_bytes)
        
        # Remove padding
        padding = codes.get('padding', 0)
        if padding > 0:
            binary = binary[:-padding]
        
        # Reverse Huffman codes (code -> value)
        reverse_codes = {v: k for k, v in codes.items() if k != 'padding'}
        ...
        return torch.tensor(decoded, dtype=torch.long).reshape(original_shape)
    
    def quantize(self, weights: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        """
        Quantize weights using AQLM with entropy coding.
        ...
        for block_indices_list in all_indices:
            ...
            for indices in block_indices_list:
                codes = self._build_huffman_tree(indices)
                encoded, meta = self._encode_indices_huffman(indices, codes)
                block_encoded.append(encoded)
                block_huffman.append(meta)
            
            encoded_indices.append(block_encoded)
            huffman_metadata.append(block_huffman)
    ...
    def dequantize(self, quantized: torch.Tensor, metadata: Dict) -> torch.Tensor:
        """
        Dequantize weights using AQLM with entropy coding.
        ...
        """
        # Decode indices from Huffman codes
        encoded_indices = metadata['encoded_indices']
        huffman_metadata = metadata['huffman_metadata']
        
        decoded_indices = []
        for block_idx, block_encoded in enumerate(encoded_indices):
            block_decoded = []
            for cb_idx, encoded in enumerate(block_encoded):
                huff_meta = huffman_metadata[block_idx][cb_idx]
                # Reconstruct original shape (block_size^2,)
                original_shape = (self.block_size * self.block_size,)
                decoded = self._decode_indices_huffman(encoded, huff_meta, original_shape)
                block_decoded.append(decoded)
