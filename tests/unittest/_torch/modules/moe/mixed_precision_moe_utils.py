...
"""Shared helpers for mixed-precision MoE tests and benchmarks.

Utility module for testing MixedPrecisionMoERunner against a reference
...
from tensorrt_llm._torch.modules.fused_moe.mixed_precision_moe_runner import (
...
# ---------------------------------------------------------------------------
# Weight packing helpers for MixedPrecisionMoERunner
# ---------------------------------------------------------------------------
...
def pack_fp4_weights_for_fused_module(
    ...
    """Pack per-expert NVFP4 weights into fused tensors for MixedPrecisionMoERunner.
...
# ---------------------------------------------------------------------------
# MixedPrecisionMoERunner creation
# ---------------------------------------------------------------------------


def create_mixed_precision_runner(
    num_experts: int,
    hidden_size: int,
    intermediate_size: int,
    top_k: int,
    ...
    ep_size: int = 1,
    ep_rank: int = 0,
) -> Tuple["MixedPrecisionMoERunner", torch.Tensor, torch.Tensor]:
    """Create and configure a MixedPrecisionMoERunner with loaded weights.

    Returns:
        (runner, bf16_fc1_packed, bf16_fc2_packed)
        The bf16 packed weights are returned because run_moe() needs them
        passed explicitly (they are not stored inside the runner).
    """
    runner = MixedPrecisionMoERunner()

    # Create per-expert precision assignment: first N experts are bf16 (1), rest are fp4 (0)
    assignment = torch.zeros(num_experts, dtype=torch.int32, device="cuda")
    assignment[:num_high_precision_experts] = 1
    runner.set_expert_precision_assignment(assignment, num_high_precision_experts)

    # Pack and set fp4 weights + quant scales
    w3_w1_fp4, w2_fp4, fp4_scales = pack_fp4_weights_for_fused_module(
        fp4_weights, num_experts, hidden_size, intermediate_size
    )
    runner.set_fp4_weights(w3_w1_fp4, w2_fp4, fp4_scales)

    # Use default GEMM tactics
    runner.set_tactic()

    # Pack bf16 weights (returned for explicit passing to run_moe)
    bf16_fc1, bf16_fc2 = pack_bf16_weights_for_fused_module(
        bf16_weights, num_experts, hidden_size, intermediate_size
    )

    return runner, bf16_fc1, bf16_fc2
...
def run_mixed_precision_forward(
    runner: "MixedPrecisionMoERunner",
    bf16_fc1: torch.Tensor,
    bf16_fc2: torch.Tensor,
    x: torch.Tensor,
    router_logits: torch.Tensor,
    routing_method: RenormalizeMoeRoutingMethod,
    tp_size: int = 1,
    ...
    """Run forward pass on MixedPrecisionMoERunner.

    Applies routing from router_logits, then calls runner.run_moe().
    """
    ...
    with torch.inference_mode():
        return runner.run_moe(
            ...
            fc1_expert_weights=bf16_fc1,
            fc2_expert_weights=bf16_fc2,
            tp_size=tp_size,
            tp_rank=tp_rank,
            ep_size=ep_size,
            ep_rank=ep_rank,
