...
def unpack_fp4_codes(packed: torch.Tensor) -> torch.Tensor:
    """Unpack FP4 codes from packed format (2 codes per byte).
    
    FP4 nibble order: LOW in even index, HIGH in odd index.
    """
    codes = []
    for i in range(0, len(packed), 2):
        byte_pair = packed[i:i+2]
        if len(byte_pair) == 2:
            low = byte_pair[0].item() & 0x0F
            high = (byte_pair[1].item() >> 4) & 0x0F
            codes.extend([low, high])
        elif len(byte_pair) == 1:
            low = byte_pair[0].item() & 0x0F
            codes.append(low)
...
def load_safetensors_weights(checkpoint_dir: Path) -> dict[str, torch.Tensor]:
...
def analyze_real_model(checkpoint_dir: Path, max_tensors: Optional[int] = None) -> dict:
    """Analyze real model weights with K-means codebook."""
    ...
    # Find FP4 quantized weights (typically BF16 in safetensors, will be quantized in forward pass)
    fp4_tensors = []
    for name, tensor in weights.items():
        # Look for weight tensors (not biases, scales, etc.)
        if "weight" in name and tensor.dtype in [torch.bfloat16, torch.float32]:
            fp4_tensors.append((name, tensor))
    
    print(f"Found {len(fp4_tensors)} potential FP4 weight tensors")
    
    if max_tensors:
        fp4_tensors = fp4_tensors[:max_tensors]
        print(f"Analyzing first {len(fp4_tensors)} tensors")
    
    # Analyze each tensor
    results = {
        "total_tensors": len(fp4_tensors),
        "tensor_analyses": [],
        "aggregate_stats": {
    ...
    for tensor_idx, (name, tensor) in enumerate(fp4_tensors):
        print(f"\n[{tensor_idx+1}/{len(fp4_tensors)}] Analyzing {name} {tensor.shape}...")
        
        # Flatten tensor
        flat = tensor.flatten().to(torch.float32)
        
        # Simulate FP4 quantization by rounding to nearest E2M1 value
        # (In real forward pass, this is done by torch.ops.trtllm.fp4_quantize)
        codes = []
        for val in flat:
        ...
        for block_idx in range(min(num_blocks, 50)):  # Sample first 50 blocks per tensor
...
def main():
    print("=" * 70)
    print("STEP 1: REAL MODEL EVALUATION")
    print("=" * 70)
    ...
    # Analyze real model
    print(f"\nAnalyzing real model from {checkpoint_dir}")
    start_time = time.time()
    results = analyze_real_model(checkpoint_dir, max_tensors=10)
    ...
    bits_3bit = 3.0 + 0.5/16  # 3 bits for code + 0.5 bits for codebook overhead per 16 elements
