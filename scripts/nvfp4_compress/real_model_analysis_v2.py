#!/usr/bin/env python3
"""Step 1: Real Model Evaluation (Version 2).
...
def main():
    print("=" * 70)
    print("STEP 1: REAL MODEL EVALUATION (VERSION 2)")
    print("=" * 70)
    ...
    # Find actual weight tensors (not scales, biases, etc.)
    fp4_tensors = []
    for name, tensor in weights.items():
        if "weight" in name and tensor.dtype in [torch.bfloat16, torch.float32]:
            # Skip scale tensors and biases
            if "scale" not in name and "bias" not in name:
                fp4_tensors.append((name, tensor))
    
    print(f"Found {len(fp4_tensors)} weight tensors")
    
    # Analyze first 10 tensors only (for speed)
    fp4_tensors = fp4_tensors[:10]
