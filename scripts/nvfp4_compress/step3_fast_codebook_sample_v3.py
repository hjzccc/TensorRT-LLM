#!/usr/bin/env python3
"""Step 3 Fast: Build K-Means Codebook Library for Sample Tensors (Version 3).

Uses local checkpoint directory directly.
"""
...
print(f"[Step 3 Fast] Build K-Means Codebook Library (Sample v3)")
...
# Find all safetensors files with proper pattern matching
safetensors_files = sorted([f for f in CKPT_DIR.glob("*") if f.name.startswith("model-") and f.name.endswith(".safetensors")])

if not safetensors_files:
    print(f"ERROR: No safetensors files found in {CKPT_DIR}")
    print(f"Contents: {list(CKPT_DIR.iterdir())}")
    sys.exit(1)
