#!/usr/bin/env python3
"""
Phase 2 Integration Tests: Final Version
...
def create_synthetic_data(
    ...
    num_samples: int = 128,
    quantization_noise: float = 0.05,  # Larger noise for realistic scenario
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
) -> tuple:
    """Create synthetic data with realistic quantization noise."""
    torch.manual_seed(42)
    np.random.seed(42)
    
    reference = torch.randn(num_experts, num_samples, hidden_size, device=device) * 0.1
    quantized = reference + torch.randn_like(reference) * quantization_noise
...
def test_phase1_only():
    ...
    reference, quantized, _ = create_synthetic_data(device=device, quantization_noise=0.05)
    ...
    assert improvement > 1.0, f"Expected >1% improvement, got {improvement:.2f}%"
...
def test_phase1_plus_phase2():
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    reference, quantized, fisher_diag = create_synthetic_data(device=device, quantization_noise=0.05)
    ...
    assert phase2_mse <= phase1_mse, "Phase 2 should not increase MSE"
...
def run_all_tests():
    """Run all tests."""
    print("=" * 80)
    print("PHASE 2 INTEGRATION TESTS (FINAL)")
    print("=" * 80)
