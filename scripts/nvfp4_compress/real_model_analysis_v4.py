#!/usr/bin/env python3
"""Step 1: Real Model Evaluation (Version 4 - With Proper FP4 Unpacking).

Load actual NVFP4 weights from safetensors and analyze with K-means codebook.
This version properly unpacks FP4 codes from uint8 packed format.
"""
...
def unpack_fp4_codes(packed_uint8: torch.Tensor) -> torch.Tensor:
    ...
    Each uint8 contains 2 FP4 codes:
    - Low nibble (bits 0-3): first code
    - High nibble (bits 4-7): second code
    """
    codes = []
...
def main():
    print("=" * 70)
    print("STEP 1: REAL MODEL EVALUATION (VERSION 4 - WITH FP4 UNPACKING)")
    print("=" * 70)
    ...
    # Analyze first 20 tensors (for better statistics)
    weight_tensor_index = weight_tensor_index[:20]
    ...
    for tensor_idx, (file_path, key) in enumerate(weight_tensor_index):
        ...
        print(f"  Unpacked {len(codes)} FP4 codes")
        
        # Analyze blocks (sample 20 blocks per tensor for speed)
        ...
        for block_idx in range(min(num_blocks, 20)):  # 20 blocks per tensor
    ...
    # Save results
    output_file = Path("/home/jerry/Documents/fork_new/TensorRT-LLM-dual-tile/scripts/nvfp4_compress/real_model_results_v4.json")
