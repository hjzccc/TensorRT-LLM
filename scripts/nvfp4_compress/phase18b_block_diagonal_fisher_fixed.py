#!/usr/bin/env python3
"""
Phase 18B: Block-Diagonal Fisher Codebook Selection (FIXED)
...
class BlockDiagonalFisherCodebookSelector:
    ...
    def compute_block_diagonal_fisher(
        ...
        If fisher_diagonal is provided, reshape it into block-diagonal structure.
        Otherwise, estimate from block statistics.
        ...
        Returns:
            Block-diagonal Fisher matrix of shape (16, 8, 8) representing
            16 blocks of 8x8 Fisher information
        """
        if fisher_diagonal is not None:
            # Use provided Fisher diagonal
            fisher_diag = fisher_diagonal.copy()
        else:
            # Estimate from block magnitude (fallback)
            fisher_diag = np.abs(block).astype(np.float32)
        
        # Reshape into block-diagonal structure
        # Each 8x8 block captures correlations within that sub-block
        block_diagonal_fisher = np.zeros((self.num_subblocks, self.subblock_size, self.subblock_size), 
                                         dtype=np.float32)
        ...
        for i in range(self.num_subblocks):
            ...
            # Extract diagonal elements for this sub-block
            subblock_diag = fisher_diag[start_idx:end_idx]
            
            # Create 8x8 block-diagonal matrix
            # Main diagonal: Fisher values
            # Off-diagonal: correlation estimates (simplified as geometric mean)
            block_matrix = np.diag(subblock_diag)
            
            # Add off-diagonal correlations (simplified: geometric mean of diagonal pairs)
            for j in range(self.subblock_size):
                for k in range(j + 1, self.subblock_size):
                    # Correlation estimate: geometric mean of diagonal elements
                    corr = np.sqrt(subblock_diag[j] * subblock_diag[k]) * 0.5
                    block_matrix[j, k] = corr
                    block_matrix[k, j] = corr
            
            block_diagonal_fisher[i] = block_matrix
        
        return block_diagonal_fisher
    
    def compute_code_importance_from_fisher(
        self,
        block: np.ndarray,
        block_diagonal_fisher: np.ndarray
    ) -> np.ndarray:
        """
        Compute importance weight for each FP4 code using block-diagonal Fisher.
        
        For each code, compute its contribution to the overall Fisher information
        across all sub-blocks.
        
        Args:
            block: 1D array of FP4 codes (128 elements)
            block_diagonal_fisher: Block-diagonal Fisher of shape (16, 8, 8)
        ...
        for i, element in enumerate(block):
            ...
            # Find which sub-block this element belongs to
            subblock_idx = i // self.subblock_size
            element_in_subblock = i % self.subblock_size
            
            # Get Fisher importance for this element
            fisher_importance = block_diagonal_fisher[subblock_idx, element_in_subblock, element_in_subblock]
            
            # Accumulate to code importance
            code_importance[nearest_idx] += fisher_importance
    ...
    def select_codebook_block_diagonal_fisher(
        ...
        """
        # Compute block-diagonal Fisher
        bd_fisher = self.compute_block_diagonal_fisher(block, fisher_diagonal)
        
        # Compute code importance weights
        weights = self.compute_code_importance_from_fisher(block, bd_fisher)
        
        # Find best subset using weighted MSE
        best_subset = None
        best_weighted_mse = float('inf')
        
        for subset in self.all_subsets:
            # Compute weighted MSE for this subset
            weighted_mse = 0.0
            ...
            for i, element in enumerate(block):
                ...
                # Find which code this element maps to
                nearest_code_idx = list(subset)[np.argmin(distances)]
                
                # Weight by Fisher importance
                error = (element - nearest_val) ** 2
                weighted_error = error * weights[nearest_code_idx]
        ...
        return list(best_subset), best_weighted_mse
    ...
    def evaluate_on_blocks(
        ...
        for i, block in enumerate(blocks):
            ...
            # Also compute diagonal Fisher baseline for comparison
            if fisher_diag is not None:
                # Diagonal Fisher baseline
                weights_diag = fisher_diag / np.sum(fisher_diag)
                best_subset_diag = None
                best_mse_diag = float('inf')
                
                for subset_cand in self.all_subsets:
                    mse_diag = 0.0
                    for j, element in enumerate(block):
                        subset_values = self.fp4_codes[list(subset_cand)]
                        distances = np.abs(subset_values - element)
                        nearest_val = subset_values[np.argmin(distances)]
                        nearest_code_idx = list(subset_cand)[np.argmin(distances)]
                        error = (element - nearest_val) ** 2
                        weighted_error = error * weights_diag[j]
                        mse_diag += weighted_error
                    
                    if mse_diag < best_mse_diag:
                        best_mse_diag = mse_diag
                        best_subset_diag = subset_cand
                
                # Compute improvement
                if best_mse_diag > 0:
                    improvement = (best_mse_diag - weighted_mse) / best_mse_diag * 100
                    results["improvement_percentages"].append(float(improvement))
                    if improvement > 0:
                        results["blocks_with_improvement"] += 1
...
def main():
    ...
    print(f"Blocks with improvement: {results['blocks_with_improvement']}/{results['num_blocks']}")
    if results.get('avg_improvement'):
    ...
    print(f"Blocks with improvement: {results_real['blocks_with_improvement']}/{results_real['num_blocks']}")
    if results_real.get('avg_improvement'):
