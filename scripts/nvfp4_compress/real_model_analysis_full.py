#!/usr/bin/env python3
"""Step 1: Real Model Evaluation (Full - All Tensors).

Analyze all 243 weight tensors from Qwen3.5-35B-A3B checkpoint.
"""
...
def main():
    print("=" * 70)
    print("STEP 1: REAL MODEL EVALUATION (FULL - ALL TENSORS)")
    print("=" * 70)
    ...
    print(f"Analyzing ALL tensors (this will take ~10-15 minutes)...\n")
    ...
    for tensor_idx, (file_path, key) in enumerate(weight_tensor_index):
        if (tensor_idx + 1) % 50 == 0:
            elapsed = time.time() - start_time
            rate = (tensor_idx + 1) / elapsed
            remaining = (len(weight_tensor_index) - tensor_idx - 1) / rate
            print(f"[{tensor_idx+1}/{len(weight_tensor_index)}] {key[:50]:50s} ({remaining:.0f}s remaining)")
        ...
        for block_idx in range(min(num_blocks, 10)):  # 10 blocks per tensor
    ...
    print(f"\nAnalysis completed in {elapsed:.2f}s ({elapsed/60:.1f} minutes)")
    ...
    # Save results
    output_file = Path("/home/jerry/Documents/fork_new/TensorRT-LLM-dual-tile/scripts/nvfp4_compress/real_model_results_full.json")
    ...
    json_results = {
        "metadata": {
            ...
            "fp4_unpacking": "proper nibble unpacking from uint8 format",
        },
        "aggregate_stats": {
        ...
        "tensor_count": len(results["tensor_analyses"]),
    }
    ...
    print(f"\nAnalyzed {len(weight_tensor_index)} weight tensors in {elapsed/60:.1f} minutes")
