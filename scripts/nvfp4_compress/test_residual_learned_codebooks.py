#!/usr/bin/env python3
"""Test residual codebook learning with learned codebooks instead of K-means."""
...
def learn_refined_codebook(values, k, num_iterations=50):
    """Learn codebook using K-means with more iterations (refined version)."""
    kmeans = KMeans(
        n_clusters=k,
        init='k-means++',
...
def test_residual_learned():
    """Test residual codebook with refined codebooks on real weights."""
    
    print("="*70)
    print("RESIDUAL CODEBOOK WITH REFINED CODEBOOKS - REAL WEIGHT TEST")
    print("="*70)
    ...
    for idx, key in enumerate(test_keys):
        ...
        # Stage 1: Primary codebook (K-means)
        t_stage1 = time.time()
        ...
        print(f"  Stage 1 (K-means): {time.time() - t_stage1:.1f}s, MSE: {primary_mse:.6f}")
        
        # Stage 2: Residual codebook (Refined K-means)
        t_stage2 = time.time()
        residuals = weight_flat - primary_recon
        residual_cb, residual_mse, residual_kmeans = learn_refined_codebook(
        ...
        print(f"  Stage 2 (Refined): {time.time() - t_stage2:.1f}s, MSE: {residual_mse:.6f}")
        
        # Stage 3: Residual-of-residual codebook (Refined K-means)
        t_stage3 = time.time()
        residuals2 = residuals - residual_recon
        residual2_cb, residual2_mse, residual2_kmeans = learn_refined_codebook(
        ...
        print(f"  Stage 3 (Refined): {time.time() - t_stage3:.1f}s, MSE: {residual2_mse:.6f}")
    ...
    if results:
        ...
        # Save results
        output_file = Path(__file__).parent / "residual_refined_codebooks_results.json"
        with open(output_file, "w") as f:
            json.dump({
                "summary": {
                    ...
                    "approach": "Residual Codebook with Refined K-means",
                },
                "results": results,
            }, f, indent=2)
        
        print(f"\nResults saved to {output_file.name}")
        print(f"\n✅ RESIDUAL WITH REFINED CODEBOOKS VALIDATED")

if __name__ == "__main__":
    test_residual_learned()
