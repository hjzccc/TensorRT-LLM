#!/usr/bin/env python3
"""NVFP4 sub-format compression with OPTIMIZED code-space pipeline.

Key optimization: Use vectorized operations for LUT application.
"""
import sys
import time
import json
...
def apply_code_lut_vectorized(codes: torch.Tensor, lut: torch.Tensor) -> torch.Tensor:
    """Apply LUT to codes using vectorized indexing on CPU."""
    # Move to CPU for indexing
    codes_cpu = codes.cpu()
    lut_cpu = lut.cpu()
    
    # Vectorized indexing
    remapped = lut_cpu[codes_cpu.long()]
    
    # Move back to original device
    return remapped.to(codes.device)
...
def nvfp4_linear_codespace(input_tensor, weight_bf16, lut, device='cuda'):
    ...
    if lut is not None:
        ...
        codes_mapped = apply_code_lut_vectorized(codes, lut)
        packed_new = repack_fp4_codes(codes_mapped)
        # Free memory
        del codes, codes_mapped
    else:
...
def evaluate(approach_name, codebook_name, lut, device='cuda'):
    ...
    print(f"Pipeline: code-space (OPTIMIZED)")
