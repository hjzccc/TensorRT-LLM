#!/usr/bin/env python3
"""NVFP4 sub-format compression with MEMORY-EFFICIENT code-space pipeline.

Key optimization: Process codes in-place without keeping full unpacked codes in memory.
"""
import sys
import time
import json
...
def remap_packed_fp4_inplace(packed: torch.Tensor, lut: torch.Tensor) -> torch.Tensor:
    """Remap packed FP4 codes using LUT, processing in-place to save memory.
    
    Args:
        packed: [M, K/2] uint8 packed FP4 codes
        lut: [16] uint8 lookup table
    
    Returns:
        [M, K/2] uint8 remapped packed codes
    """
    ...
    # Process each byte
    M, K_half = packed_cpu.shape
    remapped = torch.zeros_like(packed_cpu)
    
    for i in range(M):
        for j in range(K_half):
            byte_val = packed_cpu[i, j].item()
            low = byte_val & 0x0F
            high = (byte_val >> 4) & 0x0F
            
            # Remap using LUT
            low_new = lut_cpu[low].item()
            high_new = lut_cpu[high].item()
            
            # Repack
            remapped[i, j] = (low_new | (high_new << 4))
    
    return remapped.to(packed.device)


def nvfp4_linear_codespace(input_tensor, weight_bf16, lut, device='cuda'):
    """NVFP4 linear with code-space sub-codebook mapping (memory-efficient)."""
    ...
    if lut is not None:
        packed_new = remap_packed_fp4_inplace(packed_orig, lut)
    else:
...
def evaluate(approach_name, codebook_name, lut, device='cuda'):
    """Full model evaluation (memory-efficient)."""
    ...
    print(f"Pipeline: code-space (MEMORY-EFFICIENT)")
