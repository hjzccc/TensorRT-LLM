"""
Phase 3E: EM-Based Clustering Refinement for NVFP4 Compression (FIXED)
"""
...
class EMQuantizer:
    ...
    def quantize(self, weight: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute block means for clustering
        block_means = blocks.mean(dim=1)  # (num_blocks,)
        
        # EM clustering on block means
        centers, assignments = self._em_clustering(block_means, self.k_codes, max_iter=20)
        
        # Reconstruct
        reconstructed_means = centers[assignments]
        
        # Expand back to full blocks
        reconstructed = torch.zeros_like(blocks)
        for i in range(num_blocks):
            reconstructed[i] = reconstructed_means[i]
    ...
    def _em_clustering(self, data: torch.Tensor, k: int, max_iter: int = 20) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for iteration in range(max_iter):
            ...
            distances = torch.abs(data.unsqueeze(1) - centers.unsqueeze(0))  # (num_points, k)
            ...
            for i in range(k):
                ...
                if weights.sum() > 0:
                    new_centers[i] = (data * weights).sum() / weights.sum()
                else:
        ...
        # Hard assignments
        distances = torch.abs(data.unsqueeze(1) - centers.unsqueeze(0))
