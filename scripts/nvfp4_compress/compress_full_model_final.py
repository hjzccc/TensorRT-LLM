#!/usr/bin/env python3
"""Compress full model with RANDOM CODEBOOK - FINAL VERSION.

Saves codebooks and scales incrementally to avoid memory crashes.
"""
...
def main():
    print("=" * 80)
    print("FULL MODEL COMPRESSION (FINAL - INCREMENTAL SAVING)")
    print("=" * 80)
    sys.stdout.flush()
    ...
    for idx, (weight_name, weight_np) in enumerate(weights_to_compress.items(), 1):
        ...
        try:
            ...
            # Add to tensors for saving
            codebook_tensors[f"{weight_name}_primary"] = torch.from_numpy(result["primary_codebook"])
            codebook_tensors[f"{weight_name}_residual"] = torch.from_numpy(result["residual_codebook"])
            codebook_tensors[f"{weight_name}_residual2"] = torch.from_numpy(result["residual2_codebook"])
            
            scale_tensors[weight_name] = torch.from_numpy(result["scales"])
