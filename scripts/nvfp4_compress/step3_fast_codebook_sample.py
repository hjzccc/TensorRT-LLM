#!/usr/bin/env python3
"""Step 3 Fast: Build K-Means Codebook Library for Sample Tensors.

This script:
1. Analyzes a representative sample of weight tensors
2. Builds K-means codebook for each tensor
3. Validates the approach works at scale
4. Estimates full model compression

Expected time: 5-10 minutes for sample
"""
...
print(f"[Step 3 Fast] Build K-Means Codebook Library (Sample)")
...
print(f"✓ Total blocks: {total_blocks}")
print(f"✓ Mean MSE: {total_mse / total_blocks:.6f}")
print(f"✓ Build time: {elapsed:.1f} seconds ({elapsed/SAMPLE_SIZE:.2f}s per tensor)")
print()
print("Extrapolation to full model:")
estimated_full_time = (elapsed / SAMPLE_SIZE) * 243  # ~243 weight tensors
print(f"  Estimated time for full model: {estimated_full_time:.0f} seconds ({estimated_full_time/60:.1f} minutes)")
print()
print("Next steps:")
print("  1. Validate sample results are consistent with Step 1")
print("  2. Build full codebook library (20-30 minutes)")
print("  3. Implement inference optimization with codebook")
print()
