#!/usr/bin/env python3
"""Compress full model with uniform quantization + entropy coding (FIXED).

Uses a simpler, proven Huffman implementation.
"""
...
def uniform_quantize(values, num_levels):
    ...
    codes = np.argmin(np.abs(values.reshape(-1, 1) - codebook), axis=1).astype(np.uint8)
...
def encode_with_huffman(codes, huffman_codes):
    ...
    for code in codes:
        code_int = int(code)
        if code_int not in huffman_codes:
            raise KeyError(f"Code {code_int} not in Huffman table. Available: {list(huffman_codes.keys())}")
...
def main():
    print("=" * 80)
    print("FULL MODEL ENTROPY CODING COMPRESSION (FIXED)")
    print("=" * 80)
    ...
    metadata = {
        "approach": "Uniform Quantization + Entropy Coding (Fixed)",
