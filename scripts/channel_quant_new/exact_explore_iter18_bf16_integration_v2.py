...
Run inside trtllm-dual-tile docker:
  python /workspace/channel_quant_new/exact_explore_iter18_bf16_integration_v2.py --nsamples 4
"""
...
W1_CHANNELS = 14336  # Qwen3.5-35B-A3B MoE intermediate size
W2_CHANNELS = 8192   # Qwen3.5-35B-A3B hidden size
W1_GRANULARITY = 16
W2_GRANULARITY = 32
...
class ThreeTierConfig:
    ...
    def description(self) -> str:
        return f"BF16={self.bf16_fraction:.0%}, FP8={self.fp8_fraction:.0%}, NVFP4={self.nvfp4_fraction:.0%}"


ALL_CONFIGS = [
    ThreeTierConfig("bf16_2_fp8_48",   0.02, 0.48, 0.50),
    ThreeTierConfig("bf16_5_fp8_45",   0.05, 0.45, 0.50),
    ThreeTierConfig("bf16_10_fp8_40",  0.10, 0.40, 0.50),  # likely best
    ThreeTierConfig("bf16_15_fp8_35",  0.15, 0.35, 0.50),
]
...
def compute_channel_sensitivity(weight: torch.Tensor) -> torch.Tensor:
    """Compute per-output-channel sensitivity score using weight magnitude."""
    return weight.float().abs().mean(dim=-1)


def _snap(n: int, total: int, granularity: int) -> int:
    """Snap count to granularity constraint."""
    if n <= 0:
        return 0
    if n >= total:
        return total
...
def three_tier_moe_forward(
    ...
    for expert_idx in range(model_config.num_experts):
        w1_key = f"experts.gate_up_proj.weight"
        w2_key = f"experts.down_proj.weight"
        
        if w1_key not in tensors or w2_key not in tensors:
            continue
        
        w1_weight = tensors[w1_key][expert_idx]
        w2_weight = tensors[w2_key][expert_idx]
        w1_tier = w1_tiers[expert_idx].to(hidden_states.device)
        w2_tier = w2_tiers[expert_idx].to(hidden_states.device)

        gate_up_out = three_tier_linear(hidden_states, w1_weight, w1_tier)
        gate_up_out = F.silu(gate_up_out)
        expert_out = three_tier_linear(gate_up_out, w2_weight, w2_tier)
        final_hidden_states = final_hidden_states + expert_out
...
def evaluate_three_tier_ppl(
    ...
    """Evaluate PPL with three-tier quantization."""
    ...
    with torch.inference_mode():
        for layer_idx in range(model_config.num_hidden_layers):
            ...
            # Load expert weights for tier assignment
            expert_gate_up = []
            expert_down = []
            for expert_idx in range(model_config.num_experts):
                gate_up_key = f"experts.{expert_idx}.gate_up_proj.weight"
                down_key = f"experts.{expert_idx}.down_proj.weight"
                if gate_up_key in moe_t and down_key in moe_t:
                    expert_gate_up.append(moe_t[gate_up_key].cpu())
                    expert_down.append(moe_t[down_key].cpu())
            
            if expert_gate_up and expert_down:
                gate_up_weights = torch.stack(expert_gate_up, dim=0)
                down_weights = torch.stack(expert_down, dim=0)
                w1_tiers, w2_tiers = build_three_tier_masks(
                    gate_up_weights, down_weights, tier_config, model_config.num_experts
                )
            else:
                # Fallback: all NVFP4
                w1_tiers = {i: torch.zeros(W1_CHANNELS, dtype=torch.long) for i in range(model_config.num_experts)}
                w2_tiers = {i: torch.zeros(W2_CHANNELS, dtype=torch.long) for i in range(model_config.num_experts)}
        ...
        for sample_idx in range(nsamples):
            ...
            shift_labels = eval_chunks[sample_idx : sample_idx + 1, 1:].to(device)
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            ...
            "scope": "moe_only",
            "sensitivity_metric": "per-channel weight magnitude (absolute error)",
