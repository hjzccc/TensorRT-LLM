...
Tests different FP8 budget fractions to find the Pareto frontier between
accuracy and memory under exact TRT-LLM kernels.

Configurations (MoE experts only, attention/shared/DeltaNet stay BF16):
  1. budget_10pct  — W1=2%  FP8, W2=8%  FP8 (total ~10%)
  2. budget_20pct  — W1=4%  FP8, W2=16% FP8 (total ~20%) [current best]
  3. budget_30pct  — W1=6%  FP8, W2=24% FP8 (total ~30%)
  4. budget_40pct  — W1=8%  FP8, W2=32% FP8 (total ~40%)
  5. budget_50pct  — W1=10% FP8, W2=40% FP8 (total ~50%)
  6. budget_w2only_30pct — W1=0% FP8, W2=30% FP8 (W2-only budget)
  7. budget_w2only_50pct — W1=0% FP8, W2=50% FP8 (W2-only budget)

Run inside trtllm-dual-tile docker:
  python /workspace/channel_quant_new/exact_explore_iter02_budget_sweep.py --nsamples 4
"""
...
DEFAULT_OUTPUT = RESULTS_DIR / "exact_explore_iter02_budget_sweep.json"
...
def build_budget_masks(
    ...
    """Build FP8 channel masks for a given budget configuration."""
    ...
    # Build joint W1/W2 base masks (routing-frequency-based expert tiers)
    ...
    # Boost joint mask members, then apply global top-k with new budget
    ...
    mixed_w1_masks, mixed_w2_masks, mixed_meta = build_global_fraction_masks(
        ...
        budget_source=f"budget_sweep_{budget_config.label}",
    )
    ...
    metadata = {
        ...
        **alignment_meta,
        ...
        "w1_fp8_fraction_target": budget_config.w1_fp8_fraction,
        "w2_fp8_fraction_target": budget_config.w2_fp8_fraction,
        "w1_selected": count_selected(mixed_w1_masks),
        "w2_selected": count_selected(mixed_w2_masks),
        "score_source": "router_affinity_weighted_qerror",
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            ...
            "experiment": "fp8_budget_sweep",
            "purpose": "Find Pareto frontier between FP8 budget and PPL under exact TRT-LLM kernels",
