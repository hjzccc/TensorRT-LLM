#!/usr/bin/env python3
"""Budget sweep with per-layer calibration loading to avoid OOM."""
...
CAL_DIR = "/code/tensorrt_llm/scripts/channel_quant_new/profiling/calibration_layers"
...
def load_layer_cal(layer_idx):
    with open(Path(CAL_DIR) / f"layer_{layer_idx}.json") as f:
        return json.load(f)
...
def build_masks(cal_layer, gate_up, down, layer_idx, budget, num_experts):
    ...
    for ei in range(num_experts):
        for sk, w, n_ch, gran, out in [
            ("w1", gate_up[ei], W1_N, W1_GRAN, w1_tiers),
            ("w2", down[ei], W2_N, W2_GRAN, w2_tiers),
        ]:
            cal_e = cal_experts.get(ei)
            if cal_e and "act_weighted" in cal_e[sk]:
                scores = torch.tensor(cal_e[sk]["act_weighted"], dtype=torch.float32)
            else:
...
def evaluate(eval_ids, ns, sl, cfg, wm, sd, dev, dt, budget, label):
    store = ee.WeightStore(MODEL_ID, sd, wm)
    rt = store.load_tensors([
    ...
    with torch.inference_mode():
        for li in range(cfg.num_hidden_layers):
            ...
            ld = shorten_layer_tensors(li, raw, dev, dt)
            del raw

            mt = {k.replace("mlp.", "", 1): v for k, v in ld.items() if k.startswith("mlp.")}
            cal_layer = load_layer_cal(li)
            w1t, w2t = build_masks(cal_layer, mt["experts.gate_up_proj"], mt["experts.down_proj"], li, budget, cfg.num_experts)
            del cal_layer
            ...
            del w1t, w2t
            if (li + 1) % 10 == 0:
                print(f"  [{label}] layer {li + 1}/{cfg.num_hidden_layers}", flush=True)

        for si in range(ns):
            torch.cuda.empty_cache()
            c = chunks[si : si + 1].to(dev)
            h = hb[si : si + 1].to(dev)
...
def main():
    ...
    print(f"Calibration: per-layer from {CAL_DIR}", flush=True)
    ...
    for budget in BUDGETS:
        ...
        t0 = time.time()
        ppl = evaluate(eval_ids, ns, sl, cfg, wm, sd, dev, dt, budget, label)
        el = time.time() - t0
    ...
    out = Path(OUT_DIR) / "budget_sweep_v3.json"
    with out.open("w") as f:
        json.dump({
            "metadata": {
                "metric": "act_weighted",
                "allocation": "depth_ramp",
                "cal": "wikitext2_train_128x2048",
                "eval": "wikitext2_test_145x2048",
            },
            "results": results,
        }, f, indent=2)
    print(f"\nSaved -> {out}")
