#!/usr/bin/env python3
"""Evaluate BF16+NVFP4 with different sensitivity metrics from calibration data.

Calibration: WikiText-2 TRAIN split (calibration.json)
Evaluation: WikiText-2 TEST split (full 145 chunks)

Tests each metric at 15% BF16 budget with depth_ramp allocation:
  1. weight_mag (baseline) — ||w||₁ per channel
  2. act_weighted — E[a²] × ||w||² from calibration data
  3. output_sensitivity — E[|output_channel|] from calibration data
  4. routing_scaled_act — routing_weight × act_weighted (MoE-aware)
"""
import argparse
import json
import sys
...
CALIBRATION_PATH = "/code/tensorrt_llm/scripts/channel_quant_new/profiling/calibration.json"
...
def depth_ramp_fraction(layer_idx, base=None):
    if base is None:
        base = TARGET_BF16
    fracs = np.linspace(base * 0.33, base * 1.67, NUM_LAYERS)
    fracs = fracs * (base / fracs.mean())
    return float(fracs[layer_idx])


def build_masks_from_scores(
    w1_scores_per_expert, w2_scores_per_expert,
    gate_up_weights, down_weights,
    layer_idx, num_experts,
):
    bf16_frac = depth_ramp_fraction(layer_idx)
    w1_tiers = {}
    w2_tiers = {}

    for expert_idx in range(num_experts):
        for scores, weights, n_ch, gran, tiers_out in [
            (w1_scores_per_expert.get(expert_idx), gate_up_weights[expert_idx], W1_N, W1_GRAN, w1_tiers),
            (w2_scores_per_expert.get(expert_idx), down_weights[expert_idx], W2_N, W2_GRAN, w2_tiers),
        ]:
            if scores is None:
                tiers_out[expert_idx] = torch.zeros(n_ch, dtype=torch.long)
                continue

            scores_t = torch.tensor(scores, dtype=torch.float32)
            n_bf16 = snap(int(round(bf16_frac * n_ch)), n_ch, gran) if bf16_frac > 0 else 0
            tiers = torch.zeros(n_ch, dtype=torch.long)
            _, sorted_idx = scores_t.sort(descending=True)
            if n_bf16 > 0:
                tiers[sorted_idx[:n_bf16]] = 2

            weight_mag = weights.float().abs().mean(dim=-1).cpu()
            nvfp4_ch = (tiers == 0).nonzero(as_tuple=True)[0]
            if nvfp4_ch.numel() > 0:
                zero_mask = weight_mag[nvfp4_ch] == 0
                if zero_mask.any():
...
def three_tier_linear(x, w, tiers):
    ...
    if nvfp4_mask.any():
        ...
        output.index_copy_(1, idx, full_nvfp4.index_select(1, idx))
...
def evaluate_with_metric(
    ...
    device, dtype, calibration, metric_name, label, layer_batch_size,
):
    ...
    with torch.inference_mode():
        for li in range(config.num_hidden_layers):
            ...
            layer_cal = calibration.get(str(li), {}).get("experts", [])
            w1_scores = {}
            w2_scores = {}
            for e in layer_cal:
                ei = e["expert"]
                if metric_name == "routing_scaled_act":
                    rw = e["routing_weight_sum"]
                    w1_scores[ei] = [v * rw for v in e["w1"]["act_weighted"]]
                    w2_scores[ei] = [v * rw for v in e["w2"]["act_weighted"]]
                else:
                    w1_scores[ei] = e["w1"][metric_name]
                    w2_scores[ei] = e["w2"][metric_name]

            w1_tiers, w2_tiers = build_masks_from_scores(
                w1_scores, w2_scores,
                mt["experts.gate_up_proj"], mt["experts.down_proj"],
                li, config.num_experts,
            )
...
METRICS = [
    ...
    ("act_weighted", "act_weighted"),
    ("output_sensitivity", "output_sensitivity"),
    ("routing_scaled_act", "routing_scaled_act"),
]
...
def main():
    ...
    print(f"Calibration: {calibration['metadata']}", flush=True)
    print(f"Eval: {nsamples} chunks × {seqlen} tokens from WikiText-2 TEST", flush=True)
    print(f"Baselines: BF16={BF16_PPL}, NVFP4={NVFP4_PPL}, gap={GAP:.4f}", flush=True)

    results = {}
    for metric_name, metric_key in METRICS:
        print(f"\n=== {metric_name} (15% BF16, depth_ramp) ===", flush=True)
        t0 = time.time()
        ppl = evaluate_with_metric(
            eval_ids, nsamples, seqlen, config, weight_map, snapshot_dir,
            device, dtype, calibration, metric_key, metric_name, args.layer_batch_size,
        )
    ...
    out = Path(OUTPUT_DIR) / "metric_comparison.json"
    with out.open("w") as f:
        json.dump({"metadata": {"nsamples": nsamples, "seqlen": seqlen, "calibration": "wikitext2_train_128x2048", "budget": TARGET_BF16, "allocation": "depth_ramp"}, "results": results}, f, indent=2)
    print(f"\nSaved -> {out}")
    print(f"\n{'Metric':<25s} {'PPL':>10s} {'Recovery':>10s}")
    print("-" * 47)
    for name, r in sorted(results.items(), key=lambda x: x[1]["ppl"]):
        print(f"{name:<25s} {r['ppl']:>10.4f} {r['gap_recovery_pct']:>9.1f}%")
