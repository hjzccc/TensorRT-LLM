...
"""Eager-mode inference with pre-quantized NVFP4 weights from safetensors.

This script loads pre-quantized NVFP4 weights (packed FP4 + block scales) from
a pre-quantized checkpoint and runs layer-by-layer inference without on-the-fly
quantization overhead.

Supported baselines:
  - uniform_bf16 (on-the-fly quantization, baseline)
  - uniform_nvfp4_prequant (pre-quantized weights)
"""
...
def main() -> None:
    ...
    # Load from pre-quantized checkpoint if using prequant mode
    snapshot_dir, root_config, weight_map = load_root_config(args.model_id)
    
    # Override with pre-quantized checkpoint if needed
