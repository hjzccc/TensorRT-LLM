...
def main():
    ...
    with torch.inference_mode():
        for lidx in range(text_config.num_hidden_layers):
            ...
            for eidx in range(text_config.num_experts):
                ...
                if n_tok > 2:
                    act = flat[ti]
                    act_kurt = torch.zeros(text_config.hidden_size, device=device)
                    mu = act.mean(0)
                    std = act.std(0).clamp(min=1e-8)
                    act_kurt = ((act - mu) / std).pow(4).mean(0) - 3.0
                    act_kurt = act_kurt.abs()
                    pair_scores = (act_kurt.unsqueeze(0) * w1_err).sum(dim=1)
                    ...
                    silu_out = F.silu(g) * u
                    act_kurt_w2 = torch.zeros(text_config.moe_intermediate_size, device=device)
                    mu2 = silu_out.mean(0)
                    std2 = silu_out.std(0).clamp(min=1e-8)
                    act_kurt_w2 = ((silu_out - mu2) / std2).pow(4).mean(0) - 3.0
                    act_kurt_w2 = act_kurt_w2.abs()
                    w2_channel_scores[eidx] = (act_kurt_w2.unsqueeze(0) * w2_err).sum(dim=1).cpu()
                else:
    ...
    # Config 1: ALL FP8 for W2, ALL FP4 for W1
    w1m1: dict[int, dict[int, torch.Tensor]] = {}
    w2m1: dict[int, dict[int, torch.Tensor]] = {}
    for lidx in range(text_config.num_hidden_layers):
        w1m1[lidx] = {e: torch.zeros(text_config.moe_intermediate_size, dtype=torch.bool) for e in range(text_config.num_experts)}
        w2m1[lidx] = {e: torch.ones(text_config.hidden_size, dtype=torch.bool) for e in range(text_config.num_experts)}
    plan1 = make_plan("w2_all_fp8_w1_all_fp4", "W2=100% FP8, W1=0% FP4",
                       text_config, non_expert_bytes, total_expert_elems, w1m1, w2m1)
    run_plan(plan1, "w2_all_fp8_w1_all_fp4")

    # Config 2: Outlier protection — top 1% channels FP8
    w1m2, w2m2 = build_topk_masks(metric_cache, text_config, 0.01, 0.01)
    plan2 = make_plan("outlier_1pct", "Top 1% channels by activation_kurtosis at FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m2, w2m2)
    run_plan(plan2, "outlier_1pct")

    # Config 3: Top 5% channels
    w1m3, w2m3 = build_topk_masks(metric_cache, text_config, 0.05, 0.05)
    plan3 = make_plan("outlier_5pct", "Top 5% channels at FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m3, w2m3)
    run_plan(plan3, "outlier_5pct")

    # Config 4: W1=0% W2=20% (only protect W2)
    w1m4, w2m4 = build_topk_masks(metric_cache, text_config, 0.0, 0.20)
    plan4 = make_plan("w2_only_20pct", "W1=0% W2=20% FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m4, w2m4)
    run_plan(plan4, "w2_only_20pct")

    # Config 5: W1=0% W2=30%
    w1m5, w2m5 = build_topk_masks(metric_cache, text_config, 0.0, 0.30)
    plan5 = make_plan("w2_only_30pct", "W1=0% W2=30% FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m5, w2m5)
    run_plan(plan5, "w2_only_30pct")

    # Config 6: W1=5% W2=50% (heavy W2)
    w1m6, w2m6 = build_topk_masks(metric_cache, text_config, 0.05, 0.50)
    plan6 = make_plan("w1_5_w2_50", "W1=5% W2=50% FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m6, w2m6)
    run_plan(plan6, "w1_5_w2_50")

    # Config 7: Extreme — W1=0% W2=40% (our 20% total but ALL to W2)
    w1m7, w2m7 = build_topk_masks(metric_cache, text_config, 0.0, 0.40)
    plan7 = make_plan("w2_only_40pct", "W1=0% W2=40% FP8",
                       text_config, non_expert_bytes, total_expert_elems, w1m7, w2m7)
    run_plan(plan7, "w2_only_40pct")

    # Save results
    payload = {
        "metadata": {"model": MODEL_ID, "eval_tokens": EVAL_TOKENS, "calib_tokens": CALIB_TOKENS,
                      "note": "Iteration 6: fundamentally different approaches"},
        "results": results,
    }
    out_path = Path(__file__).parent / "results" / "exploration_iter06.json"
