...
"""
Iteration 52: RPTQ + MaCa (Residual Post-Training Quantization)
...
DEFAULT_OUTPUT_JSON = RESULTS_DIR / "proper_iter52_rptq_maca.json"
...
# RPTQ-specific: Use uniform 4K (best from Iter29)
RPTQ_CALIBRATION_TYPE = "maca_uniform_4k"
RPTQ_CALIBRATION_CHUNKS = 128
...
# Strategy: Keep residuals in FP8, weights in FP4
# This requires tracking residuals separately during quantization
...
def main() -> None:
    ...
    print(f"[Iter52] RPTQ + MaCa Calibration")
    ...
    # Build MaCa uniform 4K calibration
    print("[2/4] Building MaCa uniform 4K calibration set...")
    mix_config = CalibrationMixConfig(
        name=RPTQ_CALIBRATION_TYPE,
        length_counts=((RPTQ_CALIBRATION_LENGTH, RPTQ_CALIBRATION_CHUNKS),),
        description="Uniform 4K calibration (proven best from Iter29)",
    )
    ...
    payload: dict[str, Any] = {
        "metadata": {
            ...
            "dtype": args.dtype,
            "seed": args.seed,
            "experiment": "Iteration 52: RPTQ + MaCa (Residual Post-Training Quantization)",
    ...
    if args.exploration_md.exists():
        ...
        upsert_exploration_section(
            ...
            f"- Calibration: {RPTQ_CALIBRATION_TYPE}\n"
