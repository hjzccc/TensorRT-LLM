...
"""Iteration 39: Comprehensive Best Combination.

Tests the best combination of all improvements found so far:
1. MaCa calibration (best Hessian estimation)
...
def main() -> None:
    ...
    maca_calibration = load_calibration(MACA_CACHE_PATH, args.model_id)
    ...
    ensemble_calibration = average_calibrations(seed_calibrations, text_config) if seed_calibrations else maca_calibration
    ...
    configs = [
        ...
        ("maca_m6",              maca_calibration,      False, "MaCa calibration + M=6 (current best)"),
        # 4/6 variants
        ("standard_46",          standard_calibration,  True,  "Standard calibration + 4/6 adaptive"),
        ("maca_46",              maca_calibration,      True,  "MaCa calibration + 4/6 adaptive"),
        # Ensemble variants
    ...
    for plan_name, calibration, use_46, description in configs:
        ...
        atomic_json_dump({"results": results, "metadata": {"approach": "comprehensive_best"}}, args.output_json)
